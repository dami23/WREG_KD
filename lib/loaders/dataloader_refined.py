...
class DataLoader(Loader):
    ...
    def fetch_ann_features(self, image_id, opt):
        """Returns
        ann_pool5: float32 (1, 1024, 7, 7)
        ann_fc7: float32 (1, 2048, 7, 7)
        im_info: float32 [[im_h, im_w, im_scale]]
        """
        ann_feats_dir = 'res101_coco_minus_refer_notime_ann_fc7'
        pool_fc7_feats_dir = osp.join('/media/mi/Data/Dataset/feats/', opt['dataset_splitBy'], 'mrcn', ann_feats_dir)        
        
        feats_h5 = osp.join(pool_fc7_feats_dir, str(image_id)+'.h5')
        feats = h5py.File(feats_h5, 'r')
        ann_pool5, ann_fc7, im_info = np.array(feats['ann_pool5']), np.array(feats['ann_fc7']), feats['im_info']
        
        ann_pool5 = Variable(torch.from_numpy(ann_pool5).cuda())
        ann_fc7 = Variable(torch.from_numpy(ann_fc7).cuda())
    ...
    def getBatch(self, split, opt):
        ...
        embedmat_path = '/home/mi/projects/own_project/WREG_py3/cache/word_embedding/embed_matrix.npy'
        embedding_mat = np.load(embedmat_path)

        vocab_file = '/home/mi/projects/own_project/WREG_py3/cache/word_embedding/vocabulary_72700.txt'
        f = open(vocab_file, "r")
        ...
        sent_extract = json.load(open(osp.join('/home/mi/projects/own_project/WREG_py3/cache/sub_obj_wds', dataset + '_' + splitBy, "sent_extract.json")))
        ...
        ann_fc7, ann_pool5, lfeats = self.fetch_ann_features(image_id, opt)

        pool5 = ann_pool5.unsqueeze(0).expand(img_sent_num, ann_num, 1024, 7, 7)
        pool5.detach()
    ...
    def getTestBatch(self, split, opt):
        ...
        max_len = (labels!=0).sum(1).max().item()
